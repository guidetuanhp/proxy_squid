...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 20.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/guidetuanhp/proxy/main/squid.conf
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/guidetuanhp/proxy/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 3128 -j ACCEPT
    /sbin/iptables-save
    ...
    /bin/rm -f /etc/squid/squid.conf
    /usr/bin/touch /etc/squid/blacklist.acl
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/guidetuanhp/proxy/main/conf/squid-centos7.conf
    systemctl enable squid
    systemctl restart squid
